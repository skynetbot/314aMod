...
USE [Compliance]
GO
DROP TABLE IF EXISTS [dbo].[ofacsdn_Match_Tier1]
GO
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE TABLE [dbo].[ofacsdn_Match_Tier1](
...
INSERT INTO @allPersonsfn
	SELECT distinct concat('%',b.lastName,'%'),
		b.lastName
	FROM [Compliance].[dbo].[sdn] b where LEN(b.lastName) <= 4
INSERT INTO @allPersonsfn
	SELECT distinct concat('%',
		substring(b.lastName,1,CEILING((LEN(b.lastName)/1.5)/1.5)),'%')
		,b.lastName
	FROM [Compliance].[dbo].[sdn] b where LEN(b.lastName) > 4
WHILE @cnt <= (select count(*) from @allPersonsfn)
BEGIN
	--############### INSERT FROM CLIENTESFIRMANTEStable ###############
INSERT INTO [Compliance].[dbo].[ofacsdn_Match_Tier1]
...
INSERT INTO @allPersonsln
	SELECT distinct concat('%',b.lastName,'%'), b.lastName FROM [Compliance].[dbo].[sdn] b where LEN(b.lastName) <= 4
INSERT INTO @allPersonsln
	SELECT distinct concat('%',
		substring(b.lastName,1,CEILING((LEN(b.lastName)/1.5)/1.5)),'%')
		, b.lastName
...
INSERT INTO [Compliance].[dbo].[ofacsdn_Match_Tier1]
	SELECT a.Nombre,a.[Nombre Fantasía], CONCAT(a.[Nº Documento]
		,' ',a.[Tipo de documento]),a.[País del documento],
		b.lastName,b.first_name,b.middle_name,b.suffix
	FROM [Compliance].[dbo].[clientesFirmantes] a
	,@allPersonsln b
	WHERE a.Nombre LIKE 
		(select b.match_string
...
GO
---------2nd tier------------------------------------------
---ACCOUNTING FOR SHORT NAMES-- format lastName firstname
...
INSERT INTO @allPersonslnfn
	SELECT distinct concat('%',b.lastName,'%',b.first_name,'%'),
		b.lastName,b.first_name,b.middle_name,b.suffix 
	FROM [Compliance].[dbo].[314a_14997_MatchPerson_Tier1] b where LEN(b.lastName) <= 4 or LEN(b.first_name) <= 4
INSERT INTO @allPersonslnfn
	SELECT distinct concat('%',
		substring(b.lastName,2,CEILING((LEN(b.lastName)/1.5)/1.5)),'%',
		substring(b.first_name,2,CEILING((LEN(b.first_name)/1.5)/1.5)),'%')
		, b.lastName,b.first_name,b.middle_name,b.suffix
	FROM [Compliance].[dbo].[314a_14997_MatchPerson_Tier1] b where LEN(b.lastName) > 4 or LEN(b.first_name) > 4
--select * from @allPersonslnfn
---ACCOUNTING FOR SHORT NAMES-- format firstname lastName
...
INSERT INTO @allPersonsfnln
	SELECT distinct concat('%',b.first_name,'%',b.lastName,'%'),
		b.lastName,b.first_name,b.middle_name,b.suffix 
	FROM [Compliance].[dbo].[314a_14997_MatchPerson_Tier1] b where LEN(b.lastName) <= 4 or LEN(b.first_name) <= 4
INSERT INTO @allPersonsfnln
	SELECT distinct concat('%',
...
---ALGORITHM TESTING
SELECT distinct b.lastName, b.first_name
...
DECLARE @allPersonsfn table(
	...
	[id] [int] IDENTITY(1,1) NOT NULL)
INSERT INTO @allPersonsfn
	SELECT distinct concat('%',b.first_name,'%'), b.lastName,b.first_name,b.middle_name,b.suffix FROM [Compliance].[dbo].[314a-Person-14997] b where LEN(b.first_name) <= 4
INSERT INTO @allPersonsfn
	SELECT distinct concat('%',
...
DECLARE @allPersonsln table(
	...
	[id] [int] IDENTITY(1,1) NOT NULL)
INSERT INTO @allPersonsln
	SELECT distinct concat('%',b.lastName,'%'), b.lastName,b.first_name,b.middle_name,b.suffix FROM [Compliance].[dbo].[314a-Person-14997] b where LEN(b.lastName) <= 4
INSERT INTO @allPersonsln
	SELECT distinct concat('%',
		substring(b.lastName,1,CEILING((LEN(b.lastName)/1.5)/1.5)),'%')
		, b.lastName,b.first_name,b.middle_name,b.suffix
	FROM [Compliance].[dbo].[314a-Person-14997] b where LEN(b.lastName) > 4
...
DECLARE @allPersonslnfn table(
	...
	[id] [int] IDENTITY(1,1) NOT NULL)
INSERT INTO @allPersonslnfn
	SELECT distinct concat('%',b.lastName,'%',b.first_name,'%'), b.lastName,b.first_name,b.middle_name,b.suffix FROM [Compliance].[dbo].[314a-Person-14997] b where LEN(b.lastName) <= 4 or LEN(b.first_name) <= 4
INSERT INTO @allPersonslnfn
	SELECT distinct concat('%',
		substring(b.lastName,2,CEILING((LEN(b.lastName)/1.5)/1.5)),'%',
		substring(b.first_name,2,CEILING((LEN(b.first_name)/1.5)/1.5)),'%')
		, b.lastName,b.first_name,b.middle_name,b.suffix
	FROM [Compliance].[dbo].[314a-Person-14997] b where LEN(b.lastName) > 4 or LEN(b.first_name) > 4
...
DECLARE @allPersonsfnln table(
	...
	[id] [int] IDENTITY(1,1) NOT NULL)
INSERT INTO @allPersonsfnln
	SELECT distinct concat('%',b.first_name,'%',b.lastName,'%'), b.lastName,b.first_name,b.middle_name,b.suffix FROM [Compliance].[dbo].[314a-Person-14997] b where LEN(b.lastName) <= 4 or LEN(b.first_name) <= 4
INSERT INTO @allPersonsfnln
	SELECT distinct concat('%',
		substring(b.first_name,2,CEILING((LEN(b.first_name)/1.5)/1.5)),'%',
		substring(b.lastName,2,CEILING((LEN(b.lastName)/1.5)/1.5)),'%')
...
--SELECT distinct a.match_string, b.lastName, b.first_name, b.middle_name, b.suffix  from @allPersons a right join [Compliance].[dbo].[314a-Person-14997] b on b.lastName like a.match_string
---ACCOUNTING FOR SHORT NAMES
SET @shortName = 4
WHILE @cnt < @cnt_total
BEGIN
	
	--IF @shortName <= (SELECT LEN(b.lastName) FROM [Compliance].[dbo].[314a-Person-14997] b where b.id = @cnt)
	--	select distinct concat('%',b.lastName,'%') 
	--	from [Compliance].[dbo].[314a-Person-14997] b
	--	where b.id = @cnt and LEN(b.lastName) <= @shortName;
	--ELSE
	--	select distinct concat('%',substring(b.lastName,2,CEILING((LEN(b.lastName)/1.5)/1.5)),'%')
	--	from [Compliance].[dbo].[314a-Person-14997] b
	--	where b.id = @cnt;
	SET @cnt = @cnt + 1;
END;
...
select b.lastName,concat('%',substring(b.lastName,2,CEILING((LEN(b.lastName)/1.5)/1.5)),'%')
FROM [Compliance].[dbo].[314a-Person-14997] b 

SELECT distinct concat('%',substring(b.lastName,1,CEILING((LEN(b.lastName)/1.5)/1.5)),'%') FROM [Compliance].[dbo].[314a-Person-14997] b